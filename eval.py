...
# from model.PoseTransformerVid import TransPoseHVid
...
class Eval(Base):
    def __init__(self, phase='validate', **kwargs):
        ...
        self.model = OTPose(cfg, phase=phase)
        self.model.eval()
    ...
    def list_evaluate_model_files(self, cfg, phase):
        ...
        print(self.checkpoints_save_folder)
    ...
    def eval(self):
        ...
        if len(self.evaluate_model_state_files) == 0:
            ...
            for model_checkpoint_file in self.evaluate_model_state_files:
                ...
                mAP = self.core_function.eval(model=model, dataloader=self.dataloader, tb_writer_dict=self.tb_writer_dict, epoch=epoch,
                                              phase=self.phase)
                logger.info(mAP)
            return mAP, model
