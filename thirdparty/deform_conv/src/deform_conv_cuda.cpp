...
void modulated_deform_conv_cuda_forward(
  ...
  const int channels = input.size(1);  // input_channels
  ...
  // resize output
  output = output.view({batch, channels_out, height_out, width_out}).zero_();  // batch , channels_out , height_out , width_out
  ...
  output = output.view({output.size(0), group, output.size(1) / group,output.size(2), output.size(3)});
        //  batch_size , group , channels_out / group , heights , widths
  for (int b = 0; b < batch; b++) {
    ...
    for (int g = 0; g < group; g++) {
      output[b][g] = output[b][g]
                         .flatten(1) // 变成一维的
                         .addmm_(weight[g].flatten(1), columns[g]) // mat + weight[g].flatten(1)* columns[g]
                         .view_as(output[b][g]);
    }
